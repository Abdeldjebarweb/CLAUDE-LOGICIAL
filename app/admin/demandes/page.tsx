...
import { CheckCircle, XCircle, FileText, Download, Eye, ExternalLink, Loader2 } from 'lucide-react'
...
// Génération PDF attestation sur l'honneur
...
<head>
  ...
  <style>
    body { font-family: Arial, sans-serif; max-width: 700px; margin: 60px auto; color: #222; line-height: 1.7; }
    .header { text-align: center; border-bottom: 3px solid #006233; padding-bottom: 20px; margin-bottom: 40px; }
    .logo { font-size: 24px; font-weight: bold; color: #006233; }
    ...
    .sig-line { border-top: 1px solid #999; width: 200px; margin: 40px auto 8px; }
...
<body>
  <div class="header">
    <div class="logo">☪ AEAB</div>
    <div class="subtitle">Association des Étudiants Algériens de Bordeaux</div>
    <div style="font-size:12px; color:#888; margin-top:6px;">associationeab@gmail.com — 06 70 37 67 67</div>
  </div>
  ...
  <div class="content">
    ...
    <p>Après examen des pièces justificatives fournies (passeport et attestation d'inscription), l'AEAB <strong>valide et approuve</strong> la présente demande d'aide.</p>

    <p>La présente attestation est délivrée à l'intéressé(e) pour servir et valoir ce que de droit.</p>
  </div>

  <div class="stamp">AEAB<br/>VALIDÉ<br/>${today.split(' ').slice(-1)}</div>

  <div class="signature-area">
    <div class="sig-block">
      <div class="sig-line"></div>
      <div style="font-size:13px; color:#555;">Le/La Bénéficiaire</div>
      <div style="font-size:12px; color:#888;">${demande.first_name} ${demande.last_name}</div>
    </div>
    <div class="sig-block">
      <div class="sig-line"></div>
      <div style="font-size:13px; color:#555;">Pour l'Association AEAB</div>
      <div style="font-size:12px; color:#888;">Le Président / La Présidente</div>
    </div>
  </div>

  <div class="footer">
    Document généré le ${today} — Association des Étudiants Algériens de Bordeaux (AEAB)<br/>
    associationeab@gmail.com — 06 70 37 67 67 — Bordeaux, France
  </div>
</body>
</html>`
  ...
  a.href = url
  a.download = `attestation-${demande.first_name}-${demande.last_name}-${Date.now()}.html`
  a.click()
...
export default function AdminDemandes() {
  ...
  const handleAccepter = async (demande: any) => {
    ...
    alert(`✅ Demande acceptée !\n\nL'attestation sur l'honneur a été générée.\nOuvrez-la dans votre navigateur et faites Ctrl+P pour l'imprimer en PDF,\npuis envoyez-la par email à ${demande.email}`)
  }
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3 text-sm text-gray-700">
              <div className="grid grid-cols-2 gap-3 bg-gray-50 p-3 rounded-lg">
                ...
                <p><b>Type d&apos;aide :</b> {sel.help_type}</p>
              ...
              <div className="border border-vert-200 rounded-xl p-4 bg-vert-50">
                ...
                <div className="space-y-2">
                  ...
                  ) : <p className="text-sm text-gray-400">Aucun passeport fourni</p>}
                  ...
                  ) : <p className="text-sm text-gray-400">Aucune attestation d&apos;inscription fournie</p>}
                </div>
              </div>
            </div>
            ...
            {sel.status !== 'resolved' && sel.status !== 'closed' && (
              <div className="flex gap-3 mt-6">
                <button
                  onClick={() => handleRefuser(sel)}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-rouge-50 text-rouge font-semibold hover:bg-rouge-100 transition-colors border border-rouge-200"
                >
                  <XCircle className="w-5 h-5" /> Refuser
                </button>
                <button
                  onClick={() => handleAccepter(sel)}
                ...
                >
                  {generatingPDF ? <><Loader2 className="w-5 h-5 animate-spin" /> Génération...</> : <><CheckCircle className="w-5 h-5" /> Accepter & Générer PDF</>}
                </button>
              </div>
            )}
            ...
            {sel.status === 'resolved' && (
              <div className="mt-6">
                <div className="bg-vert-50 border border-vert-200 rounded-lg p-3 text-sm text-vert mb-3 flex items-center gap-2">
                  <CheckCircle className="w-4 h-4" /> Demande déjà acceptée
                </div>
                <button
                  onClick={() => { generateAttestationPDF(sel); setGeneratingPDF(false) }}
                  className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-vert-50 text-vert font-semibold hover:bg-vert-100 transition-colors border border-vert-200"
                >
                  <Download className="w-5 h-5" /> Re-télécharger l&apos;attestation PDF
                </button>
              </div>
            )}

            {sel.status === 'closed' && (
              <div className="mt-6 bg-gray-50 border border-gray-200 rounded-lg p-3 text-sm text-gray-500 flex items-center gap-2">
                <XCircle className="w-4 h-4" /> Demande refusée
              </div>
            )}
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-semibold hidden lg:table-cell">Documents</th>
              ...
              <tr key={r.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 hidden lg:table-cell">
                  <div className="flex gap-1">
                    {r.passport_url && <span className="text-xs bg-vert-50 text-vert px-2 py-0.5 rounded-full">Passeport ✓</span>}
                    {r.inscription_url && <span className="text-xs bg-vert-50 text-vert px-2 py-0.5 rounded-full">Inscription ✓</span>}
                    {!r.passport_url && !r.inscription_url && <span className="text-xs text-gray-400">Aucun</span>}
                  </div>
                </td>
                <td className="px-4 py-3">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${r.urgency === 'critical' ? 'bg-rouge-100 text-rouge' : r.urgency === 'high' ? 'bg-orange-50 text-orange-700' : 'bg-yellow-50 text-yellow-700'}`}>
                ...
                <td className="px-4 py-3 text-right">
                  {r.status !== 'resolved' && r.status !== 'closed' && (
                    <div className="flex gap-1 justify-end">
                      ...
                      <button onClick={() => handleAccepter(r)} className="p-1.5 rounded-lg text-vert hover:bg-vert-50" title="Accepter et générer PDF">
                        <CheckCircle className="w-4 h-4" />
                      </button>
                    </div>
                  )}
                  {r.status === 'resolved' && (
                    <button onClick={() => generateAttestationPDF(r)} className="p-1.5 rounded-lg text-vert hover:bg-vert-50" title="Re-télécharger PDF">
                      <Download className="w-4 h-4" />
                    </button>
                  )}
                  <button onClick={() => setSel(r)} className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 ml-1" title="Voir détails">
