    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <p className="text-gray-500 mb-6">
          Vous devez être connecté à votre espace membre pour candidater comme bénévole.
        </p>
        <a href="/membre" className="btn-primary">
  ...
  if (success) return (
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="text-center animate-fade-in-up">
        ...
        <h2 className="font-heading text-2xl font-bold text-gray-900">Candidature envoyée !</h2>
        <p className="text-gray-500 mt-3 max-w-md">
          L&apos;équipe AEAB va examiner votre candidature et vous contactera prochainement.
        </p>
        <a href="/membre" className="btn-primary mt-6 inline-block">
          Retour à mon espace
        </a>
      </div>
    </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div>
                ...
                <input required type="email" className="form-input bg-gray-50" value={form.email} readOnly />
              </div>
            ...
            <div>
              <label className="form-label">Domaines de bénévolat * <span className="text-gray-400 font-normal text-xs">(sélectionnez tout ce qui vous intéresse)</span></label>
            ...
            <div>
              ...
              <input className="form-input" placeholder="Ex: Week-ends, soirs en semaine..." value={form.disponibilites} onChange={set('disponibilites')} />
            </div>
            <div>
              <label className="form-label">Expérience associative (optionnel)</label>
              <textarea rows={3} className="form-input" placeholder="Vos expériences précédentes..." value={form.experience} onChange={set('experience')} />
            </div>
            <div>
              <label className="form-label">Motivation *</label>
              <textarea required rows={4} className="form-input" placeholder="Pourquoi souhaitez-vous rejoindre l'AEAB ?" value={form.motivation} onChange={set('motivation')} />
            </div>
            ...
            <button type="submit" disabled={loading || form.domaines.length === 0}
              ...
              {loading ? <><Loader2 className="w-5 h-5 animate-spin" /> Envoi...</> : <><Heart className="w-5 h-5" /> Envoyer ma candidature</>}
            </button>
            {form.domaines.length === 0 && <p className="text-xs text-center text-rouge">Sélectionnez au moins un domaine</p>}
          </form>
        </div>
      </section>
    </>
