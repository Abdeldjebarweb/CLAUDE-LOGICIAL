...
import { MessageSquare, Send, Loader2, X, Clock, CheckCircle } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-10">
      <div className="max-w-3xl mx-auto px-4">
        <h1 className="font-heading text-2xl font-bold text-gray-900 mb-6">Mes demandes d&apos;aide</h1>
          ...
          <div className="space-y-4">
            {demandes.map(d => (
              <div key={d.id} onClick={() => openDemande(d)}
                className="bg-white rounded-xl border p-5 cursor-pointer hover:-translate-y-0.5 transition-transform">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap mb-2">
                      ...
                      {d.nouveau_message && (
                        <span className="text-xs bg-vert text-white px-2 py-0.5 rounded-full flex items-center gap-1">
                          <MessageSquare className="w-3 h-3" /> Nouveau message
                        </span>
                      )}
                    </div>
